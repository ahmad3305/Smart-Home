...
std::vector<std::unordered_map<std::string, std::string>> parseRecords(const std::string& text) {
...
std::string readFile(const std::string& filePath) {
    ...
    if (!file.is_open()) {
        std::cerr << "Error: Unable to open file " << filePath << std::endl;
        return ""; // Return an empty string if the file isn't accessible
    }
...
std::vector<std::string> parseEntities(const std::string& data, const std::string& key) {
...
std::string getUserIdFromUsername(const std::string& usersFile, const std::string& username) {
    std::ifstream infile(usersFile);
    if (!infile) return "";

    std::string line, lastUserId, lastUsername;
    while (std::getline(infile, line)) {
        if (line.find("userId:") == 0) {
            lastUserId = line.substr(7);
            lastUserId.erase(0, lastUserId.find_first_not_of(" \t"));
        }
        else if (line.find("username:") == 0) {
            lastUsername = line.substr(9);
            lastUsername.erase(0, lastUsername.find_first_not_of(" \t"));
...
int main() {
    ...
    CROW_ROUTE(app, "/login").methods("POST"_method)([](const crow::request& req) {
        ...
        if (!body) {
            ...
            return crow::response(400, R"({"success": false, "error": "Invalid JSON format"})");
        }
        ...
        return crow::response(401, R"({"success": false, "error": "Invalid credentials"})");
        });
